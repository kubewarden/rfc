...
cache provided by the `cached` crate.

The default cache backend from cached crate should be `ExpiringValueCache`.
It's a in memory cache that allow defining ttl for each value individually.
